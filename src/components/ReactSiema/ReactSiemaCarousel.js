...
export default class ReactSiemaCarousel extends React.Component {
  ...
  renderDots() {
    ...
    for (let i = 0; i < dotCount; i++) {
      ...
      dots.push(
        <button key={i} onClick={() => this.onDotClick(i)} className={`slider-dot ${className}`} />
      );
    }
    return dots;
  }
      ...
      <Fragment>
        ...
        {this.props.controls !== false && (
          <div className="slider-nav text-center">
            <button className="left-arrow btn btn-link" onClick={() => this.gotoPrev()}>
              <i className="simple-icon-arrow-left" />
            </button>
            <div className="slider-dot-container">{this.renderDots()}</div>
            <button className="left-arrow btn btn-link" onClick={() => this.gotoNext()}>
